...
void algorythm(int tda[20][20], int* xstart, int* ystart) {
	...
	tda[*ystart][*xstart] = 3;
	if (*ystart + 1 < 20 && tda[*ystart + 1][*xstart] != 5 && tda[*ystart + 1][*xstart] != 3) {
		H = sqrt((*ystart + 1 - yend)*(*ystart + 1 - yend) + (*xstart - xend)*(*xstart - xend));
		F0 = stepcost + H;
		Fvalues[0] = F0;
	}
	if (*xstart + 1 < 20 && tda[*ystart][*xstart + 1] != 5 && tda[*ystart + 1][*xstart] != 3) {
		H = sqrt((*ystart - yend)*(*ystart - yend) + (*xstart + 1 - xend)*(*xstart + 1 - xend));
		F1 = stepcost + H;
		Fvalues[1] = F1;
	}
	if (*ystart - 1 >= 0 && tda[*ystart - 1][*xstart] != 5 && tda[*ystart - 1][*xstart] != 3) {
		H = sqrt((*ystart - 1 - yend)*(*ystart - 1 - yend) + (*xstart - xend)*(*xstart - xend));
		F0 = stepcost + H;
		Fvalues[0] = F0;
	}
	if (*xstart - 1 >= 0 && tda[*ystart][*xstart - 1] != 5 && tda[*ystart - 1][*xstart] != 3) {
		H = sqrt((*ystart - yend)*(*ystart - yend) + (*xstart - 1 - xend)*(*xstart - 1 - xend));
		F1 = stepcost + H;
		Fvalues[1] = F1;
	}
